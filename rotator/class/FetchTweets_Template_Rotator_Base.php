...
class FetchTweets_Template_Rotator_Base {
    ...
    protected function _getArgs( array $aArgs, array $aTemplateOptions ) {
        ...
        $_aTemplateOptions = FetchTweets_Utilities::uniteArrays( $aArgs, $aTemplateOptions, $this->_aDefaultTemplateOptions );    
    ...
    protected function _generateStyleAttribute( array $aInlineStyles ) {
        ...
        $_aOutput       = array();
        $aInlineStyle   = array_filter( $aInlineStyles );   // drop no-true elements.
    ...
    protected function _getTRBL( array $aTop, array $aRight, array $aBottom, array $aLeft ) {
        ...
        foreach( $_aTRBL as $_aSide ) {
            // null or a blank value
            if ( isset( $_aSide['size'] ) && 0 !== $_aSide['size'] && ! $_aSide['size'] ) {    
                $_iBlank++;                
            }
        }
        if ( $_iBlank >= 4 ) {
    ...
    protected function _generateAttributes( array $aAttributes ) {
        ...
        private function __generateAttributes( array $aAttributes ) {
            ...
            foreach( $aAttributes as $_sAttribute => $_sProperty ) {
                ...
                if ( empty( $_sProperty ) && 0 !== $_sProperty ) {
                    continue;    
                }
                // must be resolved as a string.
                if ( is_array( $_sProperty ) || is_object( $_sProperty ) ) {
                    continue;    
                }
                $_aOutput[] = "{$_sAttribute}='{$_sProperty}'";
            }
            return implode( ' ', $_aOutput );
